...
int ft_run(int keycode)
{
	...
	if (keycode == 100 && !collision(player()->y, player()->x, 3) && player()->died == 0) //d
	{
		ft_run_sprites();
		player()->x += 0.05;
		map()->steps += 1;
	...
	if (keycode == 119 && !collision(player()->y, player()->x, 4) && player()->died == 0) //w
	{
		ft_run_sprites();
		player()->y -= 0.05;
		map()->steps += 1;
...
void ft_run_save_lines(int keycode)
{
	if (keycode == 97 && !collision(player()->y, player()->x, 1) && player()->died == 0) //a
	{
		ft_run_sprites_inv();
		player()->x -= 0.05;
		map()->steps += 1;
	...
	if (keycode == 115  && !collision(player()->y, player()->x, 2) && player()->died == 0) //s
	{
		ft_run_sprites();
		player()->y += 0.05;
		map()->steps += 1;
